...
r = int(sys.argv[4])
layout = sys.argv[5]            # r=1: 'one'; r=2: 'same' | 'chain' | 'disj'
cnf_out = sys.argv[6] if len(sys.argv) > 6 else None
SUPP = {'one': [{0, 1}], 'same': [{0, 1}, {0, 1}], 'chain': [{0, 1}, {1, 2}], 'disj': [{0, 1}, {2, 3}]}[layout]
assert len(SUPP) == r
t = max(max(s) for s in SUPP) + 1   # supp W is inside qubits [0, t)
m = n - k
pool = IDPool(); cnf = CNF()
...
for j, l in itertools.combinations(range(n), 2):          # collisions across qubits need E_j F_l in S
    for pj, vj in ((1, A[j]), (2, B[j]), (3, C[j])):      # pj: 1=Z-syndrome->error Z? see below
        for pl, vl in ((1, A[l]), (2, B[l]), (3, C[l])):
            ...
            if l >= t:
                cnf.append([XOR2(x, y) for x, y in zip(vj, vl)]); continue
            inS = pool.id()
            c = [pool.id() for _ in range(r)]
            for q in range(n):
                tx = ej[0] if q == j else el[0] if q == l else 0
                tz = ej[1] if q == j else el[1] if q == l else 0
                for tgt, fn in ((tx, gx), (tz, gz)):
                    x = XOR([AND(c[i], fn(i, q)) for i in range(r)])
                    cnf.append([-inS, x if tgt else -x])
            cnf.append([XOR2(x, y) for x, y in zip(vj, vl)] + [inS])
for i in range(1, r):                                     # W-basis generators: weight <= 2
    s2 = [OR([a(i, j), b(i, j)]) for j in range(n)]
    cnf.extend(CardEnc.atmost(s2, bound=2, vpool=pool, encoding=EncType.seqcounter).clauses)
...
for i, sup in enumerate(SUPP):                            # W-basis generators with fixed supports
    for j in range(n):
        if j in sup: cnf.append([a(i, j), b(i, j)])
        else: cnf.extend([[-a(i, j)], [-b(i, j)]])
def le(x, y):
...
for j in range(t, n - 1):                                 # strict order outside supp W
    lt(A[j], A[j + 1])

print(f'degen2 r={r} {layout} n={n} k={k} w={w} vars={pool.top} clauses={len(cnf.clauses)}', flush=True)
if cnf_out:
