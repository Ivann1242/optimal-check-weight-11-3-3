...
theorem sat_layout_chain_10 (x z : Fin 7 → Fin 10 → Bool) (c : Fin 10 → Fin 10 → Fin 3 → Fin 3 → Fin 7 → Bool) :
    ¬ DegenLayoutSpec 4 2 3 (fun i => if i.val = 0 then {0, 1} else {1, 2}) x z c := by
  intro h
