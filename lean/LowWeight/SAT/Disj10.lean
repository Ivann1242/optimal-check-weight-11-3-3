...
theorem sat_layout_disj_10 (x z : Fin 7 → Fin 10 → Bool) (c : Fin 10 → Fin 10 → Fin 3 → Fin 3 → Fin 7 → Bool) :
    ¬ DegenLayoutSpec 4 2 4 (fun i => if i.val = 0 then {0, 1} else {2, 3}) x z c := by
  intro h
